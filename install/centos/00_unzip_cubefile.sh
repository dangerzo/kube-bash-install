...
if [ "${NETWORK_TYPE}" == "public" ]; then
  ...
  sudo yum clean all
  CONTAINERD_IO_VERSION=1.6
  INSTALL_CONTAINERD_IO_VERSION_PATCH=`sudo yum -y --showduplicates list containerd.io | grep $CONTAINERD_IO_VERSION | awk {'print $2'} | sort -t '.' -k3 -n | uniq | tail -n 1`
...
baseurl=http://${REPO_URL}/aws77rpm
gpgcheck=0
enabled=1
EOF
