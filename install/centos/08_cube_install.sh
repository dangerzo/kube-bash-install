...
EOF
  ...
  RUNC_CH_LINE=`expr ${RUNC_CH_LINE} + 1`
  sudo awk 'NR=='"${RUNC_CH_LINE}"'{print "        [plugins.\"io.containerd.grpc.v1.cri\".registry.mirrors.\"'"${HARBOR_URL}"'\"]"}1' ${CUBE_TMP}/cube/temp/runc_out_01 | sudo tee ${CUBE_TMP}/cube/temp/runc_out_02 1>/dev/null
  RUNC_CH_LINE=`expr ${RUNC_CH_LINE} + 1`
  sudo awk 'NR=='"${RUNC_CH_LINE}"'{print "          endpoint = [\"https://'"${HARBOR_URL}"'\"]"}1' ${CUBE_TMP}/cube/temp/runc_out_02 | sudo tee ${CUBE_TMP}/cube/temp/runc_out_03 1>/dev/null
  sudo cat < ${CUBE_TMP}/cube/temp/runc_out_03 | sudo tee /etc/containerd/config.toml 1>/dev/null
  sleep 3
  sudo systemctl restart containerd
  sleep 3
fi
