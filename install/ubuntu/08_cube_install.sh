...
sudo sed -i -r -e "s/\            SystemdCgroup = false/\            SystemdCgroup = true"/ /etc/containerd/config.toml
...
if [ "${NETWORK_TYPE}" == "private" ]; then
  sudo mkdir -p ${CUBE_TMP}/cube/temp/
  sudo mkdir -p /etc/containerd/certs.d/${HARBOR_URL}/
...
cat <<EOF | sudo tee /etc/containerd/certs.d/${HARBOR_URL}/hosts.toml
server = "https://${HARBOR_URL}"

[host."https://${HARBOR_URL}"]
ca = "/etc/docker/certs.d/${HARBOR_URL}/ca.crt"
capabilities = ["pull", "resolve"]
EOF

  sudo sed -i -r -e  "s/      config_path = \"\"/      config_path = \"\/etc\/containerd\/certs.d\"/" /etc/containerd/config.toml
  sudo systemctl restart containerd
  sleep 3
  ...
  RUNC_CH_LINE=`cat /etc/containerd/config.toml | sudo grep ".registry.configs" -n | sudo awk -F: '{print $1}'`
  RUNC_CH_LINE=`expr ${RUNC_CH_LINE} + 1`
  sudo awk 'NR=='"${RUNC_CH_LINE}"'{print "      [plugins.\"io.containerd.grpc.v1.cri\".registry.configs.\"'"${HARBOR_URL}"'\".tls]"}1' /etc/containerd/config.toml | sudo tee ${CUBE_TMP}/cube/temp/runc_out_01 1>/dev/null
  RUNC_CH_LINE=`expr ${RUNC_CH_LINE} + 1`
  sudo awk 'NR=='"${RUNC_CH_LINE}"'{print "        insecure_skip_verify = true"}1' ${CUBE_TMP}/cube/temp/runc_out_01 | sudo tee ${CUBE_TMP}/cube/temp/runc_out_02 1>/dev/null

  RUNC_CH_LINE=`cat ${CUBE_TMP}/cube/temp/runc_out_02 | sudo grep ".registry.mirrors" -n | sudo awk -F: '{print $1}'`
  RUNC_CH_LINE=`expr ${RUNC_CH_LINE} + 1`
  sudo awk 'NR=='"${RUNC_CH_LINE}"'{print "        [plugins.\"io.containerd.grpc.v1.cri\".registry.mirrors.\"'"${HARBOR_URL}"'\"]"}1' ${CUBE_TMP}/cube/temp/runc_out_02 | sudo tee ${CUBE_TMP}/cube/temp/runc_out_03 1>/dev/null
  RUNC_CH_LINE=`expr ${RUNC_CH_LINE} + 1`
  sudo awk 'NR=='"${RUNC_CH_LINE}"'{print "          endpoint = [\"https://'"${HARBOR_URL}"'\"]"}1' ${CUBE_TMP}/cube/temp/runc_out_03 | sudo tee ${CUBE_TMP}/cube/temp/runc_out_04 1>/dev/null
  sudo cat < ${CUBE_TMP}/cube/temp/runc_out_04 | sudo tee /etc/containerd/config.toml 1>/dev/null
  sleep 3
  sudo systemctl restart containerd
  sleep 3
fi
