...
#####################################
sleep 2
sudo openssl rand -out ~/.rnd -hex 256 #openssl 1.1.1 issue
...
if [ -z "${CA_FILE_CHECK}" ]; then
  ...
  ####################################
  ### admin.conf data ###
  sudo sudo openssl genrsa -out ${CERT_DIR}/admin.key 2048
...
if [ -z "${ETCD_CA_FILE_CHECK}" ]; then
  ...
  sudo openssl req -new -key ${CERT_DIR}/etcd/server.key -subj /CN=${HOSTNAME} | sudo openssl x509 -req -CA ${CERT_DIR}/etcd/ca.crt -CAkey ${CERT_DIR}/etcd/ca.key -CAcreateserial -out ${CERT_DIR}/etcd/server.crt -days ${CERT_VALIDITY_DAYS} -extensions v3_req_etcd -extfile ${CERT_DIR}/etcd-openssl.conf
  ...
  sudo openssl req -new -key ${CERT_DIR}/etcd/peer.key -subj /CN=${HOSTNAME} | sudo openssl x509 -req -CA ${CERT_DIR}/etcd/ca.crt -CAkey ${CERT_DIR}/etcd/ca.key -CAcreateserial -out ${CERT_DIR}/etcd/peer.crt -days ${CERT_VALIDITY_DAYS} -extensions v3_req_etcd -extfile ${CERT_DIR}/etcd-openssl.conf
